...
// Default map center set to Nairobi CBD
...
export default function OpenStreetMapPicker({ label, value, onChange, id }) {
  ...
  const select = (location, leaflet = leafletInstance.current || window.L) => {
    ...
    if (marker.current) {
      ...
      marker.current = leaflet.circleMarker(point, { radius: 8 }).addTo(map.current);
    }
  ...
  useEffect(() => {
    ...
    loadOpenStreetMap()
      .then((leaflet) => {
        ...
        leafletInstance.current = leaflet;
        ...
        if (selected) {
          marker.current = leaflet.circleMarker([point.lat, point.lng], { radius: 8 }).addTo(map.current);
        }
  ...
  return (
    <div className="place-picker">
      ...
      <button type="button" className="button ghost compact" onClick={search}>
